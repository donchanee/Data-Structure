...
/* [ 문제 2-데크 ] 데크는 큐의 전단(front)과 후단(rear)에서 모두 삽입과 삭제가 가능한 자료구조다. 헤드 노드와 테일 노드가 없는 이중연결리스트를 사용하여 아래에 정의된 데크 함수들을 구현하시오.
 ...
 ◦ 입출력 형식:
    1) 첫 번째 라인 : 연산의 개수 n
    2) 두 번째 이후 라인: n개의 연산이 한 줄에 하나씩 차례로 입력됨. - 하나의 줄에는 연산의 종류, 추가인 경우 원소가 주어짐 (원소는 양의 정수로 표기). - 연산의 종류: 다음의 연산 이름이 대문자로 주어짐. AF (add_front), AR (add_rear), DF (delete_front), DR (delete_rear), P (print)
